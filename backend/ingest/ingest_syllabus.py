...
ARCHITECTURE:
- Structured sections → StructuredAcademicStore (NO embeddings)
- Unit content → SemanticVectorStore (WITH embeddings)
- DEPARTMENT-SCOPED: Each dept has separate vector collection
- AUTO-ACRONYM GENERATION: Persists acronyms for runtime use

Run when:
- First setup
- New syllabus documents added
- Re-indexing needed

PHASE 3 Changes:
- Added department parameter (required)
- Vector DB path: vector_db/{DEPT}/syllabus/
- Collection name: {DEPT}_syllabus
- Comprehensive acronym generation (theory + lab variants)

Usage:
    python ingest_syllabus.py syllabus.md --dept CSE

Version: 3.2 (Enhanced acronym generation + lab course support)
"""
...
from models import (
    CourseMetadata, CourseObjectives, CourseOutcomes,
    CourseTextbooks, CourseReferences, CourseLabExercises,
    UnitInfo, Textbook, Reference, LabExercise, ChunkMetadata
)
